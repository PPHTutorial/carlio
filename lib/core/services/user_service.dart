...
class UserData {
  ...
  factory UserData.fromFirestore(DocumentSnapshot doc) {
    ...
    return UserData(
      ...
      name: data['name'] ?? '',
      credits: credits,
      subscriptionType: _parseSubscriptionType(data['subscriptionType'] ?? 'free'),
...
class UserService {
  ...
  Future<UserData?> getUserData() async {
    ...
    try {
      ...
      if (e.code == 'permission-denied') {
        print('UserService: Permission denied - Firestore rules may not be configured correctly');
        print('User ID: $userId');
        print('Error: ${e.message}');
        print('Please ensure Firestore rules allow authenticated users to read their own documents');
        print('Rules should be: allow read: if request.auth != null && request.auth.uid == userId;');
      } else {
  ...
  Future<void> addCredits(double amount, {bool skipIfInSpendingMode = false}) async {
    final userId = _auth.currentUser?.uid;
    if (userId == null) {
    ...
    try {
      ...
      if (skipIfInSpendingMode && userData?.isInSpendingMode == true) {
        print('UserService: Skipping credit addition - user is in spending mode');
        return;
      }
      
      final newCredits = currentCredits + amount;
      final newHasReachedMinimum = newCredits >= 5.0 || (userData?.hasReachedMinimum ?? false);
      ...
      if (e.code == 'permission-denied') {
        ...
        print('Please ensure Firestore rules allow updates: allow update: if request.auth != null && request.auth.uid == userId;');
        rethrow;
      } else {
        print('UserService: Firestore error adding credits: ${e.code} - ${e.message}');
        rethrow;
      }
    } catch (e) {
  ...
  Future<bool> useCredits(double amount) async {
    ...
    try {
      ...
      final actualAmount = userData.isInSpendingMode && userData.credits < amount
          ? userData.credits
          : amount;
      
      final newCredits = (userData.credits - actualAmount).clamp(0.0, double.infinity);
      // Reset hasReachedMinimum when credits reach 0
      final newHasReachedMinimum = newCredits > 0 ? userData.hasReachedMinimum : false;
      ...
      if (e.code == 'permission-denied') {
        ...
        print('UserService: Firestore error using credits: ${e.code} - ${e.message}');
      }
      return false;
    } catch (e) {
  ...
  Future<void> updateSubscription({
    ...
    final subscriptionString = type == SubscriptionType.monthly
        ? 'monthly'
  ...
  Future<bool> canPerformAction({bool requiresPro = false, double creditCost = 1.0}) async {
    final userData = await getUserData();
    if (userData == null) return false;
